...
// Default try-order: free providers first, paid ones (openai) last.
const DEFAULT_ORDER = ["gemini", "groq", "cerebras", "openrouter", "mistral", "openai"];
...
async function callGemini(key, { system, prompt, temperature, maxTokens, json }) {
  ...
  if (!r.ok) { const e = new Error((data && data.error && data.error.message) || ("Gemini " + r.status)); e.status = r.status; throw e; }
